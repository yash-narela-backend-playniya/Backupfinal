...
// OPTIMIZE: Lower tick rate for physics to 30Hz (33ms)
const PHYSICS_DT_MS = 33;
const PHYSICS_DT = PHYSICS_DT_MS / 1000;

export class CarRaceRoom extends Room<GameState> {
  minPlayer: number = 2;
  playerCount: number = 4;
  countdownInterval: NodeJS.Timeout | undefined;
  matchTimer: Delayed | undefined;
  updateInterval: Delayed | undefined;
  betAmount: number = 0;
  winAmount: number = 0;
  matchOptionId: string = "";
  lastStateBroadcast: number = 0;

  async onAuth(client: Client, options: any): Promise<any> {
    // Wallet check only on join, NOT every tick
  ...
  async onCreate(options: any) {
    ...
    this.onMessage("controls", (client: Client, data: any) => {
      if (this.state.gamestatus !== "playing") return;
      const car = this.state.cars.get(client.sessionId);
      if (!car || car.respawning) return;
      car.throttle = data.throttle || 0;
      car.brake = data.brake || 0;
      car.steering = data.steering || 0;
      car.nitroActive = !!data.nitro;
    });
  }
  ...
  private startMatch() {
    ...
    this.clock.setInterval(() => {
      this.state.gameTime = Date.now() - (this.state as any).gameStartTime;
    }, 1000);
    this.matchTimer = this.clock.setTimeout(
      () => this.endGame(false),
      2 * 60 * 1000
    );
    // OPTIMIZE: Lower tick-rate using physics loop at 30Hz (33ms)
    this.updateInterval = this.clock.setInterval(
      () => this.updateGame(PHYSICS_DT),
      PHYSICS_DT_MS
    );
  ...
  private updateGame(dt: number) {
    // Physics and game logic at 30Hz, broadcast at 10Hz
    ...
    if (!this.lastStateBroadcast || now - this.lastStateBroadcast > 100) {
      ...
      // this.lastStateBroadcast = now;
          ...
          // Update gameTime before broadcasting
    this.state.gameTime = now - (this.state as any).gameStartTime; 
    this.broadcastState();
    this.lastStateBroadcast = now;
    }
  }
  ...
  private checkCarCollisions() {
    // Only for 4-16 cars, O(N^2) is fine
  ...
  private broadcastState() {
    // Only send what's needed, at 10Hz
    const carsArr = Array.from(this.state.cars.values());
    const top3 = carsArr
      .filter((car) => car.lives > 0)
      .sort(
        (a, b) =>
          b.score - a.score ||
          b.laps - a.laps ||
    ...
    this.broadcast("game_state", {
      ...
      cars: carsArr.map((car) => ({
        id: car.id,
        position: { x: car.position.x, z: car.position.z },
        velocity: { x: car.velocity.x, z: car.velocity.z },
        angle: car.angle,
        health: car.health,
        laps: car.laps,
        nitro: car.nitro,
      ...
      path: this.state.path.map((p) => ({ x: p.x, z: p.z })),
      checkpoints: this.state.checkpoints.map((c, i) => ({
        x: c.position.x,
        z: c.position.z,
        index: i,
      })),
